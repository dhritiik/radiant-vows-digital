...
import { Sparkles, Minus, Plus, Calendar, Users, Camera, Music, Wine, Heart, MapPin } from "lucide-react";

type Theme = "wedding" | "party" | "wedding_live" | "anni_live";

const BACKGROUNDS = {
  wedding: "https://images.unsplash.com/photo-1583939003579-730e3918a45a?auto=format&fit=crop&q=80&w=1000",
...
const WEDDING_EVENTS = [
  { id: "mehendi", label: "Mehendi", icon: Palette, time: "Feb 14, 11:00 AM" },
  { id: "sangeet", label: "Sangeet", icon: Music, time: "Feb 14, 7:30 PM" },
  { id: "wedding", label: "Wedding Ceremony", icon: Camera, time: "Feb 15, 9:00 AM" },
  { id: "reception", label: "Reception", icon: Wine, time: "Feb 15, 8:00 PM" },
];
...
export default function TryItOutSection() {
  ...
  const [guestName, setGuestName] = useState("Mr. & Mrs. Sharma");
  const [selectedEvents, setSelectedEvents] = useState<string[]>(["wedding", "reception"]);
  ...
  const toggleEvent = (id: string) => {
    setSelectedEvents((prev) => {
      if (prev.includes(id)) {
        ...
        if (!eventGuestCounts[id]) {
          setEventGuestCounts(c => ({ ...c, [id]: 1 }));
        }
        return [...prev, id];
      }
    });
        ...
        <div className="flex flex-col lg:flex-row items-center justify-center gap-12 lg:gap-20 max-w-6xl mx-auto">
          ...
          >
            ...
            <div className="bg-card/60 backdrop-blur-md p-1.5 rounded-2xl border border-border/50 flex flex-wrap gap-1 relative">
              {(["wedding", "party", "wedding_live", "anni_live"] as const).map((t) => (
                <button
                  key={t}
                  onClick={() => {
                    setTheme(t);
                    if (t === "wedding") setGuestName("Mr. & Mrs. Sharma");
                ...
                >
                  ...
                  <span className="relative z-20 flex items-center justify-center gap-1.5">
                    {t === "wedding" && <Heart className="w-3.5 h-3.5" />}
                    ...
                    <div className="mb-6">
                      ...
                      <div className="flex flex-col gap-2.5">
                        {WEDDING_EVENTS.map((event) => {
                          ...
                          const count = eventGuestCounts[event.id] || 1;
                          return (
                            <div
                              key={event.id}
                              className={`flex items-center justify-between px-3 py-2 rounded-xl border transition-all duration-300 ${isSelected
                                  ? "bg-gold/10 border-gold/50 shadow-sm"
                                  : "bg-background border-border/40 hover:border-gold/30 hover:bg-gold/5"
                                }`}
                            >
                              <button
                              ...
                              >
                                <event.icon className={`w-4 h-4 ${isSelected ? "text-gold" : "opacity-60 text-muted-foreground"}`} />
                                <span className={`font-sans text-sm font-semibold ${isSelected ? "text-gold" : "text-muted-foreground"}`}>
                                  {event.label}
                                </span>
                              </button>
                              ...
                              <AnimatePresence>
                                {isSelected && (
                                  <motion.div
                                    ...
                                    className="flex items-center gap-3 bg-background/80 rounded-lg p-1 border border-gold/20 shadow-sm"
                                  >
                                    <button
                                    ...
                                    <span className="font-display font-bold text-base min-w-[1.2rem] text-center text-foreground">
                                      {count}
                                    </span>
                                    <button
                                      onClick={() => updateCount(event.id, 1)}
                                      className="w-7 h-7 flex items-center justify-center rounded-md hover:bg-muted text-gold transition-colors"
                                    >
                      ...
                      <div className="relative z-10 h-full flex flex-col p-6 pt-16">
                        {theme === "wedding" ? (
                          <>
                            ...
                            >
                              <span className="font-sans text-[10px] tracking-[0.4em] uppercase text-gold">With Blessings</span>
                            </motion.div>

                            <div className="text-center mb-auto">
                              <p className="font-body text-cream/70 text-sm mb-2">We joyfully invite</p>
                              <h3 className="font-display text-2xl font-bold text-white leading-tight mb-8">
                                {guestName || "Guest Name"}
                              </h3>

                              <p className="font-sans text-[10px] tracking-widest uppercase text-cream/60 mb-4">Your Events</p>
                              <div className="flex flex-col gap-2.5">
                                <AnimatePresence>
                                  {selectedEvents.length === 0 && (
                                    <motion.p
                                      initial={{ opacity: 0 }}
                                      animate={{ opacity: 1 }}
                                      className="text-xs text-cream/40 italic"
                                    >
                                      No events selected
                                    </motion.p>
                                  )}
                                  {WEDDING_EVENTS.filter((e) => selectedEvents.includes(e.id)).map((event) => {
                                    const count = eventGuestCounts[event.id] || 1;
                                    return (
                                      <motion.div
                                        key={event.id}
                                        ...
                                        className="bg-black/40 backdrop-blur-sm border border-white/10 rounded-xl p-3 flex items-center justify-between gap-3"
                                      >
                                        <div className="flex items-center gap-3">
                                          ...
                                          <div className="flex flex-col text-left">
                                            ...
                                            <span className="font-sans text-[10px] text-white/50 mt-0.5">
                                              {event.time}
                                            </span>
                                          </div>
                                        </div>
                                        <div className="flex flex-col items-end border-l border-white/10 pl-3">
                                          <span className="font-display font-bold text-lg text-gold leading-none">{count}</span>
                                          <span className="font-sans text-[9px] uppercase tracking-widest text-white/60 mt-1">
                                            {count > 1 ? "Seats" : "Seat"}
                                          </span>
                                        </div>
                                      </motion.div>
